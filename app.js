...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  localStorage.setItem(KEY,JSON.stringify(data));if(!status.lastSavedAt){status.lastSavedAt=new Date().toISOString();persistStatus()}createRecoverySnapshot('Update 7.8.38 opened',data);
  loadExternalFields();renderAll();setMode('IN');
  if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=7.8.38',{updateViaCache:'none'}).then(r=>r.update()).catch(()=>{}));
});
